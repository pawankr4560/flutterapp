...
class LoanDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final liveCustomer = ref
        .watch(loanCustomersProvider)
        .firstWhere((item) => item.id == customer.id, orElse: () => customer);
    return Scaffold(
  ...
  Future<void> _collectEmi(
    BuildContext context,
    WidgetRef ref,
    LoanCustomer liveCustomer,
  ) async {
    final amount = await showDialog<double>(
      context: context,
      builder: (_) => RepaymentDialog(
        initialAmount: liveCustomer.loanDetails.nextEmiAmount,
      ),
    );
    if (amount != null) {
        ...
        return Wrap(
          ...
          children: [
            for (final metric in metrics)
              SizedBox(
                width: width,
                child: _metricCard(context, metric.$1, metric.$2),
              ),
          ],
        );
      },
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: AppSpacing.xxs),
                Text(
                  '${money(loan.nextEmiAmount)} due on ${date(loan.nextEmiDate)}',
                  style: AppTextStyles.bodyMedium(context),
                ),
              ],
            ),
